...
listen(State) ->
    receive
		...
		shutdown -> 			%%for quit response = shutdown
...
do_join(State, Ref, ChatName) ->
    ...
    %% {{dummy_target, dummy_response}, State}.
		...
		case maps:is_key(ChatName, State#cl_st.con_ch) of
			%%whereis(server) = server pid
			true -> {err, State};
			false ->
				whereis(server)!{self(), Ref, join, ChatName},
				receive
					{Server_pid, Ref, err_nick_used} ->
						{err_nick_used, State};
					{Chatroom_pid, Ref, connect, History} ->
						Added_chat = maps:put(ChatName, Chatroom_pid, State#cl_st.con_ch),
						{History, State#cl_st{con_ch = Added_chat}}
						%% DONE. message received, update records of connected chatrooms with new joined one, send back {result, self(), Ref, History} to the GUI.
				end
		end.
    ...
    %%{{dummy_target, dummy_response}, State}.
		case maps:is_key(ChatName, State#cl_st.con_ch) of
			...
			true ->
				...
				receive
					{Server_pid, Ref, ack_leave} -> %%should i do self() instead of ClientPID? or not because what i need is coming from the server?
						Removed_chat = maps:remove(ChatName, State#cl_st.con_ch),
						{ok, State#cl_st{con_ch = Removed_chat}} %%should i use something other than result?
				end
